...
$default{ 'bind_addr' } = "0.0.0.0"; # Bind address
...
$default{ 'compare_offline' } = 0; # logout during compare_dirs()?
$default{ 'pool_listings' } = '';  # regex for files listing paths in pool.  
$default{ 'pool_mappings' } = 's:^Filename\: ::'; # regex converts to get_patt
...
@boolean_values = ( 'get_newer', 'get_size_change', 'do_deletes',
	...
	'local_dir_check', 'compare_offline');
...
sub msg_version
{
	...
	&msg( 'Debian patch version: mirror (2.9-52)  Tue Oct  5 14:05:54 2004 UTC.' . "\n" );
...
sub do_mirror
{
	...
	# Add paths found in $pool_listings to %pool_file.
	undef %pool_file;
	if( $pool_listings ){
		local( $f );
		for( @local_sorted ){
			next unless m|$pool_listings|;
			# Uncompress $f if gzipped.
			$f = $_;
			if( $f =~ s/\.($gzip_suffix|$old_gzip_suffix)$// &&
				&sys( "$gzip_prog -cd <$_ >$f" ) != 0 ){
				&msg( "Cannot uncompress $_\n to $f\n" );
			};
			open( PL, $f );
			for( <PL> ){
				next unless eval $pool_mappings;
				chomp( $_ );
				$pool_file{ $_ } = 1;
			};
			close( PL );
                        &msg( "Get pool files in: $_\n" );
		};
	};
	
	local( $con_exit ) = &con_login();
        return $con_exit if $con_exit > 0;
	...
	while( @transfer_list ){
		...
		# disconnect for single connection, Debian bug #178054.
		if( $compare_offline ){
			&disconnect();
	        };
		...
		if( $compare_offline ){
			...
			return $con_exit if $con_exit > 0;
                        if( ! &ftp'cwd( $remote_dir ) ){
                                &msg( "Cannot change to remote directory ($remo\te_dir) because: $ftp'response\n");
                        }
		}
...
# Connect, login and setup FTP server 
sub con_login
{
        local( $con ) = &connect();
	if( $con <= 0 ){
...
sub patch_ls_lR_file
{
	...
	local( $to, $tn, $p, $s, $trz, $t, $m, $tlz, $tlb );
	$frb = $frt = $ls_lR_file;
	$flb = $dirtmp;
	...
	if( $frt =~ /$times_suffix$/ ){
		...
		( $p, $s, $trz, $t, $m ) = &lsparse'line( ftp'NS );
	...
	if( $tlb == $to && ! &sys( "$f $flb $flp" ) ){
		...
		$fr = "$frb.$gzip_suffix";
		$f = "$flb.$gzip_suffix";
		if( &ftp'get( $fr, $f, 0 ) && ( $s = -s $f ) &&
			! &sys( "$gzip_prog -df $f" ) ){
			&utime( $tn, $tn, $flb );
			&msg( $log, "Got $fr $s for $flb\n" );
		}
		else {
			&msg( "Did not get $fr\nand $ftp'response\n" );
			return 0;
...
sub parse_remote_details_real
{
	...
	while( 1 ){
		while( !eof( $rls ) ){
			...
			# If $pool_listings ignore remote non-pool files.
			if( $pool_listings && $get_file &&\
				( $type eq 'f' || $type eq 'l' )){
				next unless $pool_file{ $path };
				&msg( "Pool file: $path\n" ) if $debug > 1;
			};
